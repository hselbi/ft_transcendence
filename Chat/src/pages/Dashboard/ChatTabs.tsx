...
const TabsList = React.forwardRef<HTMLDivElement, TabsListProps>((props, ref) => {
    ...
    return (
        <BaseTabsList
            ref={ref}
            className={clsx(
                "rounded-t-xl mx-1 bg-transparent flex font-sans items-center justify-center content-between min-w-tabs-list",
                className,
            )}
            {...other}
        />
...
const TabPanel = React.forwardRef<HTMLDivElement, TabPanelProps>((props, ref) => {
    ...
    return (
      <BaseTabPanel
        ref={ref}
        className={clsx(
          'py-5 px-3 bg-white dark:bg-[#806EA9] border-slate-200 dark:border-slate-700 rounded-md w-full h-full font-sans text-sm',
          className,
        )}
        {...other}
      />
...
const ChatTabs = () => {
    ...
    return (
        <div className={isDarkMode ? 'dark' : ''}>
            <Tabs defaultValue={0}>
                <TabsList>
                    <Tab value={0}><UserCircle size={19} /></Tab>
                    <Tab value={1}>All</Tab>
                    <Tab value={2}>Private</Tab>
                    <Tab value={3}>Channels</Tab>
                </TabsList>
                <TabPanel value={0}>friends messages</TabPanel>
                <TabPanel value={1}><All /></TabPanel>
                <TabPanel value={2}><Privates /></TabPanel>
                <TabPanel value={3}><Channels /></TabPanel>
            </Tabs>
        </div>
    )
}
                    ...
                    return {
                        ...
                        className: clsx(
                            `font-sans ${ownerState.selected
                                ...
                                : 'cursor-pointer'
                            } text-sm leading-[1.3] font-semibold w-full py-2.5 px-3 m-1 mb-0 border-0 rounded-t-md flex justify-center focus:outline-0 focus:shadow-outline-purple-light`,
